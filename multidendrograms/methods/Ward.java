 ...
 * @since JDK 6.0
 */
public class Ward extends Method {

	public Ward(final DistancesMatrix distMatrix) {
		super(distMatrix);
	}

	@Override
	public MethodName getMethodName() {
		return MethodName.WARD;
	}
	...
	protected double getBeta(final Cluster cI, final Cluster subci, final Cluster subck, final Cluster cJ) {
		double res = -((double)cJ.getNumLeaves() / (double)cI.getNumLeaves())
				* (double)(subci.getNumLeaves() + subck.getNumLeaves())
				/ (double)(cI.getNumLeaves() + cJ.getNumLeaves());
		return res;
	}
